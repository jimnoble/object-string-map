...
{
    [TestClass]
    public class ObjectStringMapperTests
    {
        ...
        static IObjectStringMapper<TObject> GetMapper<TObject>()
            where TObject : new()
        {
            return new ObjectStringMapper<TObject>();
        }
        ...
        {
            var mapper = GetMapper<TSimpleType>();

            var template = string.IsNullOrWhiteSpace(format) ?
                "alfa/{this}/bravo" :
                $"alfa/{{this:{format}}}/bravo";

            var str = mapper.Map(template, value);
            ...
            where TSimpleType : new()
        {
            ...
            var obj = mapper.Map(template, str);
        ...
        static void MapObject_WhenWrappedType_ThenSuccess<TType>(
            ...
            where TType : new()
        {
            var mapper = GetMapper<Wrapper<TType>>();
            ...
            var str = mapper.Map(template, obj);
